...
func GetAllServiceAreaService(ctx context.Context, params utils.PaginationParams) ([]models.FeatureCollection, error) {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			logger.Info("No service area found in the database to show.")
			return nil, nil
		}
		logger.Error(fmt.Sprintf("Error retrieving service area from the database: %v", err))
		return nil, fmt.Errorf("error fetching service area details: %w", err)
	}
	defer cursor.Close(ctx)

	// Decode the results into a slice
	var serviceArea []models.FeatureCollection
	if err = cursor.All(ctx, &serviceArea); err != nil {
	...
	logger.Info(fmt.Sprintf("Retrieved %d serviceArea(s) successfully with pagination and sorting and filters.", len(serviceArea)))
